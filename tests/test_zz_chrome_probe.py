    ...
    for n in range(40):
        with tempfile.TemporaryDirectory() as profile, tempfile.TemporaryFile() as log:
            ...
            proc = subprocess.Popen(
                [binary, "--headless=new", "--window-size=1280,900", "--remote-debugging-port=0",
                 f"--user-data-dir={profile}", "--no-first-run", "--disable-popup-blocking",
            ...
            if not ok:
                ...
                report.append(subprocess.run(["ps", "-eo", "pid,stat,etime,args"], capture_output=True, text=True).stdout)
            proc.kill()
            proc.wait()
